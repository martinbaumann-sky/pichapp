...
- Crea `apps/web/.env.local` (o en la raíz `.env.local`) y añade todas las variables secretas.
- Ejemplo mínimo:
...
TWILIO_FROM=
RESEND_API_KEY=
RESEND_FROM_EMAIL="PichangApp <no-reply@pichangapp.cl>"
...
- Revisa `apps/web/src` para ver qué variables son necesarias (`process.env.*`).
