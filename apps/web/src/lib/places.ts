...
export async function searchPlace(text: string): Promise<PlaceResult[]> {
  logger.debug({ text }, "Buscando lugares");
  ...
  try {
    ...
    logger.debug({ url: osmUrl }, "Consultando OpenStreetMap");
    ...
    if (osmRes.ok) {
      ...
      logger.debug({ count: osmData.length }, "Resultados OpenStreetMap");
      ...
      const osmResults = osmData.map((item) => {
        ...
        const comuna = candidates.find((v) => comunasRM.includes(v)) || extractComunaFromText(item.display_name);
    ...
    if (hasSportsKeyword) {
      ...
      logger.debug({ url: sportsUrl }, "Búsqueda deportiva OpenStreetMap");
      ...
      if (sportsRes.ok) {
        ...
        logger.debug({ count: sportsData.length }, "Resultados deportivos OpenStreetMap");
    ...
    if (exactMatches.length > 0) {
      logger.debug({ count: exactMatches.length }, "Coincidencias exactas encontradas");
    ...
    logger.debug({ count: finalResults.length }, "Resultados finales de búsqueda");
    
    return finalResults;
    
  } catch (error) {
    logger.error({ error }, "Error en búsqueda de lugares");
