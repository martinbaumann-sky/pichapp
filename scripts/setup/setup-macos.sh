...
ENV_SAMPLE="$PROJECT_ROOT/apps/web/env-example.txt"
...
if [ -f "$ENV_SAMPLE" ] && [ ! -f "$ENV_FILE" ]; then
  printf '\nNo existe apps/web/.env. Copiando env-example.txt como base...\n'
  cp "$ENV_SAMPLE" "$ENV_FILE"
  printf 'Recuerda actualizar apps/web/.env con tus credenciales reales.\n'
fi

printf '\nDependencias instaladas correctamente.\n'
printf 'Ya puedes ejecutar ./scripts/dev/start-macos.sh para iniciar el entorno de desarrollo.\n\n'
