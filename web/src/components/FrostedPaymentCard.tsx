...
export default function FrostedPaymentCard({ amountCLP, onPayMP, onPayWebpay, onClose }: Props) {
  return (
      ...
      <div className="relative overflow-hidden rounded-3xl p-6" style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.12)", boxShadow: "0 6px 30px rgba(0,0,0,0.35)" }}>
        <div className="relative z-10 text-white">
          ...
          <div className="space-y-3">
            <button onClick={onPayMP} className="w-full px-4 py-3 bg-black text-white rounded shadow">Pagar con Mercado Pago</button>
            <button onClick={onPayWebpay} className="w-full px-4 py-3 bg-black text-white rounded shadow">Pagar con Webpay</button>
          </div>
        </div>
      </div>
    </div>
